...
export default function SubmissionCard({
  ...
  const renderDoc = (key: DocumentKey, label: string, helperNote: string, f?: FileMeta | null) => {
      ...
      <div className="rounded-lg border border-border-default bg-white p-3 sm:p-4">
        <div className="flex flex-col gap-2">
          <div className="min-w-0 w-full">
            ...
            {isEditing && helperNote && (
              <p className="text-xs font-semibold text-orange-500">{helperNote}</p>
            )}
          </div>
  ...
  return (
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-border-default bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-2">
            {renderDoc("bankPassbook", "Bank Passbook Copy", 
                "If you have a fully online bank account that does not have a passbook, you should provide a screenshot of your account profile page.", 
                group.documents?.bankPassbook)}
    ...
    <Modal isOpen={showConfirmationModal} onClose={() => setShowConfirmationModal(false)} showCloseButton={false}>
        <div className="flex flex-col items-center">
            <h2 className="text-lg font-semibold mb-4">Confirm Submission</h2>
            <p>{`Are you sure you want to submit the documents and details for recruitment series: ${group.recSeriesName}?`}</p>
            <div className="flex w-full justify-center gap-x-4 mt-4">
                <button
                    className="px-4 py-2 bg-gray-300 text-black rounded-md disabled:cursor-not-allowed disabled:opacity-60"
                ...
                <button
                    className="px-4 py-2 bg-primary text-white rounded-md flex items-center justify-center gap-2  disabled:cursor-not-allowed disabled:opacity-60"
                    disabled={isSaving}
                    onClick={async () => {
