...
const TARequestCard: React.FC<TARequestCardProps> = ({
  ...
  return (
    <div className="max-w-md mx-auto bg-bg-card rounded-xl shadow-lg overflow-hidden border border-border-default mb-6">
              ...
              return (
                <li key={idx} className="flex items-center justify-between py-3">
                  <div className="flex items-center min-w-0 flex-1 space-x-3">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${ta.status.toLowerCase() === 'accepted' ? 'bg-success/20 text-success' : ta.status.toLowerCase() === 'rejected' ? 'bg-error/20 text-error' : 'bg-warning/20 text-warning'}`}>{ta.status}</span>
                  </div>
...
const HandleTARequests = () => {
  ...
  return (
    <div className="py-8 px-4">
      <TARequestCard {...moduleInfo} onAccept={handleAccept} onReject={handleReject} />
    </div>
  )
}
