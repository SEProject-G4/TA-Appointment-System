...
const AutoSelect: React.FC<AutoSelectProps> = ({ options, placeholder = 'Select or type', className, selectedOption, onSelect }) => {
  ...
  const inputValue = isOpen ? query : (selectedOption ? selectedOption.label.toString() : '');
      ...
      <div className="relative mt-1">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'ArrowDown') {
              e.preventDefault();
              setActiveOptionIndex((prev) => Math.min(prev + 1, filteredOptions.length - 1));
              ...
              filteredOptions.map((option, index) => (
                <div
                  ...
                  className={`relative cursor-pointer select-none py-2 w-full px-5  
                    ${activeOptionIndex === index ? 'bg-primary-dark/70 text-text-inverted' : selectedOption?.id === option.id ?'bg-primary-dark/20 text-text-primary' :'bg-bg-card/80 text-text-primary'} `}
                  ...
                  ref={activeOptionIndex === index ? (el => {
                    if (el) {
