...
describe('Lecturer Routes Integration', () => {
  ...
  describe('GET /api/lecturer/modules/with-ta-requests', () => {
    it('returns modules with accepted TAs and document status', async () => {
      const modules = [
        { _id: 'm1', moduleCode: 'CS101', moduleName: 'Intro', semester: 1, year: 2025, requiredTACount: 2, requiredTAHours: 6 }
      ];
      ModuleDetails.find.mockReturnValue({ select: () => modules });

      const apps = [
        { _id: 'a1', moduleId: 'm1', userId: 'u1', status: 'accepted' },
        { _id: 'a2', moduleId: 'm1', userId: 'u2', status: 'pending' }
      ];
      TaApplication.find.mockResolvedValue(apps);

      User.find.mockReturnValue({ select: () => ([{ _id: 'u1', name: 'Alice', indexNumber: 'E/00/001', role: 'undergraduate' }]) });
      TaDocumentSubmission.find.mockReturnValue({ select: () => ({ lean: jest.fn().mockResolvedValue([{ userId: 'u1', documents: { cv: { submitted: true } }, status: 'submitted' }]) }) });

      // For aggregation counts
      TaApplication.aggregate = jest.fn().mockResolvedValue([{ _id: 'm1', total: 2 }]);
      ...
      expect(res.body.modules[0].acceptedTAs[0]).toEqual(expect.objectContaining({ name: 'Alice', docStatus: 'submitted' }));
    });
  });
});
