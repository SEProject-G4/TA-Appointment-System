...
describe('Auth Middleware', () => {
  ...
  describe('protected middleware', () => {
    ...
    test('should return 401 when no session userId', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ error: 'Not authorized, no session ID' });
    ...
    test('should return 404 when user not found', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ error: 'Not authorised, user not found' });
      expect(next).not.toHaveBeenCalled();
    });
    ...
    test('should log session debug information', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Session debug:', {
        sessionId: 'test-session-id',
        userId: 'user123',
  ...
  describe('authorize middleware', () => {
    ...
    test('should return 403 when user does not have required role', () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Forbidden: You do not have the required role' });
      expect(next).not.toHaveBeenCalled();
    });
